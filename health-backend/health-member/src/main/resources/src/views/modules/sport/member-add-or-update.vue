<template>
  <el-dialog
    ...
    <el-form-item label="用户头像图片" prop="avatar">
      <el-input v-model="dataForm.avatar" placeholder="用户头像图片"></el-input>
    </el-form-item>
    <el-form-item label="微信登录openid" prop="openId">
      <el-input v-model="dataForm.openId" placeholder="微信登录openid"></el-input>
    </el-form-item>
    <el-form-item label="微信登录会话KEY" prop="sessionKey">
      <el-input v-model="dataForm.sessionKey" placeholder="微信登录会话KEY"></el-input>
    </el-form-item>
    </el-form>
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/sport/member/info/${this.dataForm.id}`),
              method: 'get',
              ...
              if (data && data.code === 0) {
                ...
                this.dataForm.sessionKey = data.member.sessionKey
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/sport/member/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
